---
name: "L1SLOAD guide: Advanced Data Structures"
index: 7
...
This article is a continuation of our previous discussion on `l1sload`, where we introduced its basic usage. Now, we will explore how to use `l1sload` to read more complex storage structures, such as mappings, structs, and both static and dynamic arrays.

Solidity abstracts state storage from developers to offer a better experience, but in order to use `l1sload`, it’s crucial to understand how Solidity manages storage at a lower level. This guide covers various cases, complete with examples.
...
To retrieve an element, you can simply add the array index to the array's slot. The formula looks like this:

```
VALUE_SLOT = ARRAY_SLOT + VALUE_INDEX
```

Keep in mind, the length of a static array is not stored on-chain. It's only visible at compile time.

Another important note: this approach works for types larger than 16 bytes (e.g., `uint256`, `address`, `uint200`). For smaller types like `bool` or `uint8`, Solidity applies data optimizations, which we’ll cover later in this guide.
...
Unlike static arrays, dynamic arrays can grow or shrink after deployment. Because of this, Solidity stores them differently on-chain.
...
The storage slot for a dynamic array holds its length. The actual data is stored starting at `KECCAK256(ARRAY_SLOT)`, with elements stored sequentially just like in static arrays.

To query a specific element or the array length, we use the following formula:

```
VALUE_SLOT = KECCAK256(ARRAY_SLOT) + VALUE_INDEX
```
...
Consider a dynamic array of `uint104` values. Since two `uint104` values can fit into a single 32-byte storage slot (with 48 bits, or 6 bytes, remaining unused), Solidity packs these values efficiently into one slot.
...
```
VALUE_SLOT = KECCAK256(ARRAY_SLOT) + VALUE_INDEX / (SLOT_SIZE / VALUE_SIZE)
```
...
```
VALUE = SLOT_VALUE >> ((VALUE_INDEX % (SLOT_SIZE / VALUE_SIZE)) * VALUE_SIZE))
```
...
```
https://latexeditor.lagrida.com/#google_vignette

\text{ARRAY_VALUE} = \text{SLOT_VALUE} >> (\text{VALUE_INDEX } \% \frac{\text{SLOT_SIZE}} {\text{VALUE_SIZE}}) * \text{VALUE_SIZE }
```
...
For example, reading from a mapping of arrays requires combining the formulas for mappings and dynamic arrays. The formula for mappings from the previous article, and the dynamic array formula from this article, can be combined like this:

```
KECCAK256(MAPPING_KEY, SLOT)
```

Data is stored in a pattern as illustrated below:

```
KECCAK256(SLOT) + ARRAY_INDEX
...
```
VALUE_SLOT = KECCAK256(KECCAK256(MAPPING_KEY, MAPPING_SLOT)) + ARRAY_INDEX
```
