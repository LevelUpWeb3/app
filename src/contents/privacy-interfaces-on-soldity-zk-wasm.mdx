...
component main = Multiplier();`} language="cpp"/>

Now compile it and generate the artifacts that we will use later.

<ContentCopy code={`circom myCircuit.circom --r1cs --wasm --sym
...
snarkjs zkey export verificationkey myCircuit_0001.zkey verification_key.json`} language="bash"/>
...
<ContentCopy code={`snarkjs zkey export solidityverifier myCircuit_0001.zkey verifier.sol`} language="bash"/>
...
```
forge create --rpc-url https://sepolia-rpc.scroll.io/ --private-key <PRIVATE_KEY> --constructor-args <VERIFIER_ADDRESS> CircomCustomLogic.sol:CircomCustomLogic
```
...
<ContentCopy code={`npm install -g lite-server #para instalar
lite-server #para levantar el servidor`} language="bash"/>
